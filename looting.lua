...
local otuiLoaded = false

TargetBot.Looting.setup = function()
  if not otuiLoaded then
    g_ui.importStyleFromString(lootingOtui)
    otuiLoaded = true
  end
...
TargetBot.Looting.update = function(data)
  ...
  ui.maxDangerPanel.value:setText(tostring(data['maxDanger'] or 10))
  ui.minCapacityPanel.value:setText(tostring(data['minCapacity'] or 100))
  TargetBot.Looting.updateItemsAndContainers()
  dontSave = false
...
TargetBot.Looting.save = function(data)
  ...
  data['maxDanger'] = tonumber(ui.maxDangerPanel.value:getText()) or 0
  data['minCapacity'] = tonumber(ui.minCapacityPanel.value:getText()) or 0
...
TargetBot.Looting.process = function(targets, dangerLevel)
  ...
  local openedContainers = g_game.getContainers()
  local lootContainers = TargetBot.Looting.getLootContainers(openedContainers)
  ...
  for index, container in pairs(openedContainers) do
...
TargetBot.Looting.getLootContainers = function(openedContainers)
