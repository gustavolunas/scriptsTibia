...
local function isLockTargetValid(creature)
  ...
  if not cpos or not me or cpos.z ~= me.z then
    return false
  end
...
local function clearLockTarget()
  lockState.target = nil
  lockState.lastSeen = 0
  targetCache.target = nil
  targetCache.name = nil
  targetCache.lastCheck = 0
end

local lockMacro = macro(100, "Travar Target", function()
...
local function getCachedTarget()
  ...
  if not lockMacro:isOn() then
    if (currentTime - targetCache.lastCheck) < targetCache.cacheTime then
      if targetCache.target and targetCache.name and isLockTargetValid(targetCache.target) then
    ...
    local current = g_game.getAttackingCreature()
    if current and isLockTargetValid(current) then
      targetCache.target = current
      targetCache.name = current:getName()
...
onAttackingCreatureChange(function(creature, oldCreature)
  ...
  if not lockState.target and creature and isLockTargetValid(creature) then
    lockState.target = creature
    lockState.lastSeen = now or 0
  end
end)

onCreatureDisappear(function(creature)
  if creature and lockState.target == creature then
    clearLockTarget()
  end

  if creature and targetCache.target == creature then
    targetCache.target = nil
    targetCache.name = nil
    targetCache.lastCheck = 0
  end
